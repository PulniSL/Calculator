from tkinter import Tk, Entry, Button, StringVar  # Corrected the import
from tkinter import messagebox  # Optional, for handling errors gracefully

class Calculator:
    def __init__(self, master):  # Fixed `def__init__` to `def __init__`
        master.title("Calculator")
        master.geometry('357x420+0+0')  # Fixed `357*420` to `357x420`
        master.config(bg='gray')
        master.resizable(False, False)

        self.equation = StringVar()  # To store and display the input and result
        self.entry_value = ''
        ...
        Button(width=11, height=4, text='=', relief='flat', bg='#FFC0CB', command=self.solve).place(x=270, y=350)
        Button(width=11, height=4, text='C', relief='flat', bg='white', command=self.clear).place(x=0, y=350)
    
    def show(self, value):
        """Update the equation field with button clicks."""
        self.entry_value += value
        self.equation.set(self.entry_value)
    
    def clear(self):
        """Clear the equation field."""
        self.entry_value = ''
        self.equation.set(self.entry_value)
    
    def solve(self):
        """Evaluate the equation entered."""
        try:
            result = eval(self.entry_value.replace('x', '*'))  # Evaluate the expression
